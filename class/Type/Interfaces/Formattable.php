 ...
 *
 * Chaque type dispose d'un ou plusieurs formats d'affichage et d'options qui
 * lui sont propres pour paramétrer finement l'affichage souhaité.
 *
 * Par exemple, pour une date, on pourra choisir le format d'affichage à
 * utiliser (AAAA, AAAA-MM-JJ, etc.) et indiquer sous forme d'option la langue
 * souhaitée pour afficher les mois en toutes lettres.
 *
 * La méthode {@link getAvailableFormats()} permet d'obtenir la liste des
 * formats disponibles pour un type donné et {@link getDefaultFormat()} permet
 * de connaître le nom du format par défaut.
 *
 * Pour permettre à l'utilisateur de paramétrer lui-même un type, on peut
 * utiliser la méthode {@link getFormatSettingsForm()} qui retourne un
...
interface Formattable
{
     ...
     *
     * Par défaut, il s'agit du nom du premier format retourné par la méthode
     * {@link getAvailableFormats()}, ou null si celle-ci retourne un tableau
     * vide.
     *
