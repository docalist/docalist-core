 ...
 *
 * Les vues permettent de séparer (design pattern SOC) la préparation des
 * données (contrôleur) de leur représentation (vue).
 *
 * Les vues sont prédéfinies par les plugins mais peuvent être surchargées
 * par le thème en cours. Pour cela, un système de nom symbolique de la
 * forme "{plugin}:{path}" est utilisé pour désigner les vues (par exemple
 * "docalist-core:table/edit" pour désigner la vue "table/edit" du plugin
 * docalist-core).
 *
...
class Views {
     ...
     * à la vue.
     *
     * Chacun des éléments sera disponible dans la vue comme une variable
     * locale :
     *
     *     render('docalist-core:table/edit', ['table' => 'test'])
     *     >>> /table/edit.php : echo $table;
     *
     * Il est possible également de passer dans le tableau la variable 'this'
     * avec une instance d'un objet existant. Dans ce cas, la vue aura accès
     * à $this et sera exécutée comme s'il s'agissait d'une méthode de l'objet
     * (elle a accès à toutes les propriétés et méthodes de l'objet, y compris
     * celles qui sont protected ou private).
     *
     * La variable $view est une variable réservée. A l'intérieur d'une vue,
     * c'est un tableau qui contient les paramètres d'exécution de la vue :
     *
     * Par exemple :
     * array(
     ...
     * cours, ce qui permet de surcharger les vues par défaut fournies par les
     * plugins en fournissant une version spécialisée, puis dans le répertoire
     * "views" du plugin.
     *
